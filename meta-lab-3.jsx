...
function runMeta(studies, method="random") {
  ...
  const d=valid.map(s=>{
    const es=+s.es,lo=+s.lo,hi=+s.hi,se=(hi-lo)/(2*1.96),w=1/(se*se);
  ...
  return {studies:d,k,Q:+Q.toFixed(3),Qpval:+Qpval.toFixed(4),I2:+I2.toFixed(1),I2desc,tau2:+tau2.toFixed(5),
    pES:+pES.toFixed(4),pSE:+pSE.toFixed(4),lo95:+(pES-1.96*pSE).toFixed(4),
    hi95:+(pES+1.96*pSE).toFixed(4),pval:+pval.toFixed(4),z:+z.toFixed(3),
    method,W:+W.toFixed(4),tau:+Math.sqrt(tau2all).toFixed(4),
    fixed:{es:+fixES.toFixed(4),se:+fixSE.toFixed(4),lo:+(fixES-1.96*fixSE).toFixed(4),hi:+(fixES+1.96*fixSE).toFixed(4)},
    random:{es:+ranES.toFixed(4),se:+ranSE.toFixed(4),lo:+(ranES-1.96*ranSE).toFixed(4),hi:+(ranES+1.96*ranSE).toFixed(4),tau2:+tau2all.toFixed(5)},
    hksj, predInt};
}
...
function eggersTest(studies) {
  ...
  // Approximate two-tailed p from t (using normal approx for simplicity when df large)
...
const btnS=(v="primary")=>({
  ...
  transition:"all 0.15s cubic-bezier(0.4,0,0.2,1)",letterSpacing:0.3,
...
function ProgressBar({done,total,color}){
  ...
  return(<div style={{display:"flex",alignItems:"center",gap:10}}>
    <div style={{flex:1,background:C.brd,borderRadius:99,height:5,overflow:"hidden"}}>
      <div style={{width:`${pct}%`,height:"100%",background:barColor,borderRadius:99,transition:"width 0.4s cubic-bezier(0.4,0,0.2,1)"}}/>
    </div>
  ...
  return(<div style={{display:"flex",minHeight:"100vh",background:C.bg,fontFamily:"'IBM Plex Sans',sans-serif",color:C.txt}}>
    <style>{`
      ...
      *{box-sizing:border-box;margin:0;padding:0;}
      ...
      ::-webkit-scrollbar-thumb{background:${C.brd};border-radius:99px;}
      ::-webkit-scrollbar-thumb:hover{background:${C.muted};}

      /* Inputs */
      input,textarea,select{color-scheme:dark;transition:border-color 0.15s,box-shadow 0.15s;}
      ...
      /* Links */
      a{text-decoration:none;color:${C.acc};transition:opacity 0.12s;}
      a:hover{opacity:0.8;}
      ...
      .prog-bar{transition:width 0.4s cubic-bezier(0.4,0,0.2,1),background 0.3s;}
        ...
        <div style={{maxWidth:640,margin:"72px auto",textAlign:"center"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,textAlign:"left"}}>
            {[
              ...
              <div key={c.ph} style={{
                background:C.card,border:`1px solid ${C.brd}`,borderRadius:10,padding:16,
